...
class User_model extends MY_Model
{
    ...
    {
        $query = $this->db->select('*')->from('user')->where(array('username' => $username, 'password' => $password, 'is_active' => true))->get();
        return $query->row_array();
    }

    public function find_id_and_key($id, $key)
    {
        $query = $this->db->select('*')->from('user')->where(array('id' => $id, 'key' => $key, 'is_active' => true))->get();
        return $query->row_array();
    }
