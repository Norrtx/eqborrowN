...
class Membertype_model extends MY_Model {
	public function __construct(){
		parent::__construct();
		$this->_table = 'membertype';
		$this->delete_db = TRUE;
	...
	public function find_with_page($param){
        ...
        $condition = "created_on = '{$curr_login}'";
		if(!empty($keyword)){
	...
	public function find_name($name){
		$query = $this->db->select('id')->from('membertype')->where(array('name'=>$name))->get();
