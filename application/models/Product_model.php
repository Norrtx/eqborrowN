...
class Product_model extends MY_Model
{
    ...
    public function find_with_page($param)
    {
        ...
        $condition = "p.created_on = '{$curr_login}' and ";
        $condition .= "1=1";
        if (!empty($keyword)) {
    ...
    {        
        $this->db->select('p.*,s.id as serial_id, s.code as serial_code, s.quantity as serial_quantity');
        $this->db->from('product p');
    ...
    public function export_stock($param)
    {
        $keyword = $param['keyword'];                
        $this->db->select('p.*, c.name as category_name, s.id as serial_id, s.code as serial_code, s.quantity as serial_quantity, u.name as unit_name');
