		...
		'columns': [{
			...
			},
			{
				...
				render: function (data, type, row) {
					...
					var btnDelete = '<a href="#" data-href="' + gUrl + 'api/categories/' + data + '" data-id="' + data + '" data-name="' + dataName + '" role="button" class="btn btn-outline-danger btn-sm btn-delete"><i class="fa fa-trash"></i> ' + gDelete + '</a>';
	...
	$('#ajaxModal').on('shown.bs.modal', function (e) {
		$('#modalForm').validate({
			submitHandler: function (form) {
				axios.post(gUrl + 'api/categories', app.item, {headers:{'api-key': gApiKey}})
			...
			messages: {
				name: {
					remote: 'ชื่อหมวดหมู่ {0} มีใช้ไปแล้ว!',
				}
			},
			errorElement: 'span',
